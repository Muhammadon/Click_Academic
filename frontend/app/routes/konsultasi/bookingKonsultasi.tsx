...
export default function BookingKonsultasi() {
  ...
  const [session] = useState<TypeApi>({
    ...
    description:
      "Membahas arsitektur backend menggunakan Laravel, pembuatan REST API, integrasi database MySQL, hingga pembungkusan aplikasi menggunakan Docker agar siap di-deploy oleh tim DevOps.",
    category: "Pelatihan",
    status: "available",
  });
  ...
  const [bookingStep, setBookingStep] = useState<
    "idle" | "created_order" | "waiting_payment" | "success"
  >("idle");
  ...
  const handlePaymentInitiation = () => {
    ...
    setTimeout(() => {
      ...
      const mockSnapToken =
        "b789123-abc-" + Math.random().toString(36).substring(2, 7);
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-[var(--color-putih-bersih)] rounded-2xl p-6 shadow-sm border border-[var(--color-abu-perak)]">
              ...
              <div className="flex items-center space-x-3 bg-[var(--color-mint-lembut)] p-3 rounded-xl border border-[var(--color-sage)] mb-6">
                ...
                <div>
                  <p className="text-xs text-[var(--color-forest-khaki)] font-medium">
                    Mentor / Pemateri
                  </p>
                  <p className="text-sm font-bold text-[var(--color-hijau-botol)]">
                    {session.mentor}
                  </p>
                </div>
              </div>

              <h4 className="text-sm font-bold text-[var(--color-dark-slate)] mb-2">
                Deskripsi Kelas / Konsultasi:
              </h4>
            ...
            <div className="bg-[var(--color-cream)] border border-[var(--color-soft-ochre)] rounded-xl p-4 flex space-x-3">
              ...
              <div className="text-xs text-[var(--color-dark-slate)] leading-relaxed">
                <span className="font-bold">Ketentuan Pembatalan:</span> Dana
                komitmen (commitment fee) yang telah dibayarkan melalui sistem
                Midtrans tidak dapat ditarik kembali jika mahasiswa membatalkan
                secara sepihak, kecuali jadwal diubah atau dibatalkan oleh pihak
                mentor terkait.
              </div>
            </div>
          </div>
            ...
            <div className="bg-[var(--color-putih-bersih)] rounded-2xl p-6 shadow-sm border border-[var(--color-abu-perak)] sticky top-24 space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-extrabold text-[var(--color-hijau-botol)]">
              ...
              {bookingStep === "success" && (
                <div className="p-4 bg-emerald-50 border border-[var(--color-hijau-zamrud)] rounded-xl text-center space-y-3">
                  ...
                  <div>
                    <h4 className="font-bold text-sm text-[var(--color-hijau-botol)]">
                      Pembayaran Sukses!
                    </h4>
                    <p className="text-[11px] text-[var(--color-forest-khaki)] mt-1">
                      Status booking di tabel telah diubah menjadi{" "}
                      <strong>'paid'</strong>.
                    </p>
                  </div>
